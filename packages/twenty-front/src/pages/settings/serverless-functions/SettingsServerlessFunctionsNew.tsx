...
export const SettingsServerlessFunctionsNew = () => {
  ...
  } = useCreateOneServerlessFunction();
  ...
  useHotkeyScopeOnMount(SettingsServerlessFunctionHotkeyScope.ServerlessFunctionNew);
  useScopedHotkeys([Key.Enter], () => {
    if (canSave !== false) {
      handleSave();
    }
  }, SettingsServerlessFunctionHotkeyScope.ServerlessFunctionNew, [canSave]);
  useScopedHotkeys([Key.Escape], () => {
    navigate(getSettingsPagePath(SettingsPath.ServerlessFunctions));
  }, SettingsServerlessFunctionHotkeyScope.ServerlessFunctionNew);
  return <SubMenuTopBarContainer title={t("New Function")} links={[{
    ...
    children: t("New")
  }]} actionButton={<SaveAndCancelButtons isSaveDisabled={!canSave} onCancel={() => {
    navigate('/settings/functions');
  }} onSave={handleSave} />}>
      <SettingsPageContainer>
        <SettingsServerlessFunctionNewForm formValues={formValues} onChange={onChange} />
      </SettingsPageContainer>
